...
in the SQL Assignment One doc file.
...
-- 2. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.Product table, excludes the rows that are 0 for the column ListPrice
SELECT ProductID, Name, Color, ListPrice
FROM Production.Product
WHERE ListPrice != 0;
...
SELECT (Color + ' ' + Name) AS color_and_name
FROM Production.Product
WHERE Color IS NOT NULL;

--7. Write a Query that generates the example result set in the Assignment One doc file.
...
-- Author's Note: used SELECT * due to no explicit column retrieval
SELECT *
FROM Production.Product
WHERE Name LIKE 'S%';
...
-- Author's Note: effectively querying for Products that have Red and Black as their Color while having a ProductSubCategoryID 1,
--                or all Products that have a ListPrice between 1000 and 2000.
